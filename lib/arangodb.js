...
var ArangoDBConnector, Connector, GeoPoint, _, _fieldIncluded, arangojs, async, chalk, debug, generateConnObject, initializeDataSource, merge, qb, url,
...
ArangoDBConnector = (function(superClass) {
  ...
  ArangoDBConnector.prototype.getGraph = function(name, cb) {
    var graph, graphString;
    if (!this.db) {
      throw new Error('ArangoDB connection is not established');
    }
    graphString = ArangoDBConnector.graph;
    graph = this.db[graphString](name);
    return graph.get(cb);
  };
  ...
  ArangoDBConnector.prototype.createGraph = function(name, settings, cb) {
    var graph, graphString;
    if (!this.db) {
      throw new Error('ArangoDB connection is not established');
    }
    graphString = ArangoDBConnector.graph;
    graph = this.db[graphString](name);
    return graph.create(settings, cb);
  };
  ...
  ArangoDBConnector.prototype.create = function(model, data, options, callback) {
    ...
    return this.execute(model, 'save', data, (function(_this) {
      return function(err, result) {
        ...
        data['verified'] = true;
        return callback(err, idValue, null, {
          data: data
        });
      };
    })(this));
      ...
      return this.db.get((function(_this) {
        return function(err, db) {
          if (db) {
            ...
            _this.db.useDatabase(_this.settings.databaseName);
            return _this.automigrateModels(models, cb);
          } else {
            debug('db not found. creating db with name ' + _this.settings.databaseName);
            return _this.db.useDatabase('_system').createDatabase(_this.settings.databaseName, function(err, dbCreated) {
              if (err || !dbCreated) {
                return cb(err);
              }
              _this.db.useDatabase(_this.settings.databaseName);
  ...
  ArangoDBConnector.prototype.automigrateModels = function(models, cb) {
    if (this.debug) {
        ...
        return collection.drop(function(err) {
          if (err) {
            if (err.response.body != null) {
              ...
              if (!(err.error === true && err.errorNum === 1203 && err.errorMessage === 'unknown collection \'' + collectionName + '\'')) {
